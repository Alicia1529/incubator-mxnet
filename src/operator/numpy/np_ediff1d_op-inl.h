...
inline void copyArr(DType* dest, DType* src, size_t count,
...
void EDiff1DForward(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_TYPE_SWITCH(out_data.type_flag_, DType, {
    ...
    if (param.to_begin_arr_given) {
      ...
      Kernel<set_to_val<DType>, xpu>::Launch(s, 1, out_data.dptr<DType>(),
                                             param.to_begin_scalar.value());
      padding += 1;
    }
    ...
    if (param.to_end_arr_given) {
      ...
      Kernel<set_to_val<DType>, xpu>::Launch(s, 1, out_data.dptr<DType>() + padding + in_size,
                                             param.to_end_scalar.value());
    }
