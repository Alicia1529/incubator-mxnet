...
struct NumpyNanToNumParam : public dmlc::Parameter<NumpyNanToNumParam> {
  ...
  DMLC_DECLARE_PARAMETER(NumpyNanToNumParam) {
    ...
    .describe("Value to be used to fill positive infinity values. If no value is"
              "passed then positive infinity values will be replaced with a very"
              "large number.");
    DMLC_DECLARE_FIELD(neginf)
    .set_default(dmlc::optional<double>())
    .describe("Value to be used to fill negative infinity values. If no value is"
...
void NumpyNanToNumOpForward(const nnvm::NodeAttrs& attrs,
  ...
  if (!common::is_float(in_data.type_flag_)) return copy(s, out_data, in_data);
