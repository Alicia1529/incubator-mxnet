...
def test_np_nan_to_num():
    ...
    for [hybridize, src_dtype, copy, param] in combinations:
        ...
        if param[0] !=None:
            ...
            np_out = _np.nan_to_num(x1, copy, param[0], param[1], param[2])
            mx_out = np.nan_to_num(x3, copy, param[0], param[1], param[2])
        else:
            test_np_nan_to_num = TestNanToNum(copy=copy)
            np_out = _np.nan_to_num(x1, copy)
            mx_out = np.nan_to_num(x3, copy)
        ...
        if not (hybridize== False and copy == False):
            if hybridize:
