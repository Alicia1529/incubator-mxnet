...
def test_npi_boolean_assign():
    class TestBooleanAssignScalar(HybridBlock):
        ...
        def hybrid_forward(self, F, a, mask):
            return F.np._internal.boolean_mask_assign_scalar(a, mask, self._val, self._start_axis, out=a)
    ...
    class TestBooleanAssignTensor(HybridBlock):
        ...
        def hybrid_forward(self, F, a, mask, value):
            return F.np._internal.boolean_mask_assign_tensor(a, mask, value, self._start_axis, out=a)
        ...
        for config in configs:
            ...
            for i in range(len(dshape)):
                if i < start_axis:
                    ...
                    vshape.append(dshape[i])
            vshape = tuple(vshape)
            for val in [np.array(42.), np.array([42.]), np.random.uniform(size=vshape)]:
